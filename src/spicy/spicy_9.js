...
export const repeat = (fn, n, ...params) => {
    let arrayReturn = new Array(n);
    for (let i = 0; i < n; i++){
        arrayReturn.push(fn(params[i]));
    }
};
...
export const hasExactly = (arr, test, n) => {
    let myObject = filter(arr,test);
    if (myObject.pass.length === n){
        return true;
    }
    return false;
};
